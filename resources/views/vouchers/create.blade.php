<x-app-layout page-title="New Voucher">
@php
...
document.addEventListener('alpine:init', () => {
    Alpine.data('createVoucherPage', () => ({
        ...
        f: {
            voucher_no:             @json(old('voucher_no', '')),
            voucher_date:           @json(old('voucher_date', now()->format('Y-m-d'))),
            ...
            particular:             @json(old('particular', '')),
            remarks:                @json(old('remarks', '')),
            source_of_fund:         @json(old('source_of_fund', '')),
            or_ref:                 @json(old('or_ref', '')),
            change_amount:          @json(old('change_amount', '')),
        ...
        init() {
            const seed = @json(array_values($oldEntries) ?: []);
...
<div x-data="createVoucherPage" class="voucher-form-page flex min-h-0 min-w-0 flex-1 flex-col gap-5 overflow-y-auto">

    {{-- Back --}}
    <a href="{{ route('vouchers.index') }}"
       class="inline-flex w-fit items-center gap-1.5 text-[12px] font-medium text-slate-500 transition hover:text-omet-blue">
    ...
    <form id="voucherForm" method="POST" action="{{ route('vouchers.store') }}" enctype="multipart/form-data"
        ...
        <div class="mt-5 rounded-xl border border-slate-200 border-t-4 border-t-slate-300 bg-white shadow-sm">
            ...
            <div class="px-4 py-5 sm:px-6">
                ...
                <p class="mb-3 text-[10.5px] text-gray-400" x-text="keptAttachments.length ? 'PDF, images, Word or Excel · max 10 MB each' : 'PDF, images, Word or Excel · max 10 MB each · at least one file is required'"></p>
                <input type="file" name="attachments[]" multiple :required="keptAttachments.length === 0"
                ...
                <div class="flex shrink-0 items-center gap-2">
                    ...
                    <button type="submit"
                        ...
                        @if (auth()->user()->isAccounting())
                            ...
                            <span class="hidden sm:inline">Submit for Approval</span>
                        @else
